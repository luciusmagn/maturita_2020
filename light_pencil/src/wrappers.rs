...
impl<'r, 'a, 'b: 'a> Request<'r, 'a, 'b> {
    ...
    pub fn url_adapter(&self) -> MapAdapter {
    	let map = self.app.url_map.read().unwrap();
        map.bind(self.host(), self.path(), self.query_string(), self.method())
    }

    pub fn match_request(&mut self) {
    	let map = self.app.url_map.read().unwrap();
        let url_adapter = map.bind(self.host(), self.path(), self.query_string(), self.method());
