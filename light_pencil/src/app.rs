...
pub struct Pencil {
    ...
    pub url_map: RwLock<Map>,
    view_functions: RwLock<HashMap<String, ViewFunc>>,
...
impl Pencil {
    pub fn new(root_path: &str) -> Pencil {
        Pencil {
            ...
            url_map: RwLock::new(Map::new()),
            view_functions: RwLock::new(HashMap::new()),
    ...
    pub fn route<M: Into<Matcher>, N: AsRef<[Method]>>(&self, rule: M, methods: N, endpoint: &str, view_func: ViewFunc) {
        self.add_url_rule(rule.into(), methods.as_ref(), endpoint, view_func);
    }

    pub fn get<M: Into<Matcher>>(&self, rule: M, endpoint: &str, view_func: ViewFunc) {
        self.route(rule, &[Method::Get], endpoint, view_func);
    }

    pub fn post<M: Into<Matcher>>(&self, rule: M, endpoint: &str, view_func: ViewFunc) {
        self.route(rule, &[Method::Post], endpoint, view_func);
    }

    pub fn delete<M: Into<Matcher>>(&self, rule: M, endpoint: &str, view_func: ViewFunc) {
        self.route(rule, &[Method::Delete], endpoint, view_func);
    }

    pub fn patch<M: Into<Matcher>>(&self, rule: M, endpoint: &str, view_func: ViewFunc) {
        self.route(rule, &[Method::Patch], endpoint, view_func);
    }

    pub fn put<M: Into<Matcher>>(&self, rule: M, endpoint: &str, view_func: ViewFunc) {
        self.route(rule, &[Method::Put], endpoint, view_func);
    }

    pub fn add_url_rule(&self, matcher: Matcher, methods: &[Method], endpoint: &str, view_func: ViewFunc) {
        let url_rule = Rule::new(matcher, methods, endpoint);
        let mut map  = self.url_map.write().unwrap();
        let mut view = self.view_functions.write().unwrap();
        map.add(url_rule);
        view.insert(endpoint.to_string(), view_func);
    }

    pub fn enable_static_file_handling(&mut self) {
        let rule = self.static_url_path.clone() + "/<filename:path>";
        self.route(&rule as &str, &[Method::Get], "static", Box::new(send_app_static_file));
    }
    ...
    fn dispatch_request(&self, request: &mut Request) -> PencilResult {
        if let Some(ref routing_error) = request.routing_error {
        	...
        	let view = self.view_functions.read().unwrap();
            match view.get(&request.endpoint().unwrap()) {
                Some(ref view_func) => view_func(request),
